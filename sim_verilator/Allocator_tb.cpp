...
int main()
{
	...
	auto dump_name = "Allocator.vcd";
	typedef VAllocator TopType;

	// Init dut and signals
	// TOP is the default name of our macro
	unique_ptr<TopType> TOP(new TopType);
	TOP->eval();
	MAP_SIGNAL_ALIAS(lbw, LBW);
	MAP_SIGNAL(blkdone_dval);
	MAP_SIGNAL(i_sizes);
	MAP_SIGNAL_ALIAS(capacity, capacity_r);
	MAP_SIGNAL(alloc_rdy);
	MAP_SIGNAL(alloc_ack);
	MAP_SIGNAL(linear_rdy);
	MAP_SIGNAL(linear_canack);
	MAP_SIGNAL(free_dval);
	MAP_SIGNAL(i_alloc_id);
	MAP_SIGNAL(o_linear);
	MAP_SIGNAL(o_linear_id);
	MAP_SIGNAL(i_free_id);
