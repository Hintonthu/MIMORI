...
int main()
{
	...
	auto dump_name = "ChunkAddrLooper.vcd";
	typedef VChunkAddrLooper TopType;
	...
	MAP_SIGNAL(i_maddr);
	MAP_SIGNAL(i_wrap);
	MAP_SIGNAL(o_cmd_type);
	MAP_SIGNAL(o_cmd_islast);
	MAP_SIGNAL(o_cmd_addr);
	MAP_SIGNAL(o_cmd_addrofs);
	MAP_SIGNAL(o_cmd_len);
